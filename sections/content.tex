...
The second last layer is the bottleneck layer with a size of 42 we then use in training for the LID Neural Network. This BNF Vectors are stacked with a context to provide the input for the 2\textsuperscript{nd} LID network, as language identity is considered to be a long-term property of the audio data. We tried different context sizes (see Sec.~\ref{sec:LIDNetwork:Input}), choosing the most robust one.  In opposition to the previous work mentioned, we then use the output layer of the 2\textsuperscript{2nd} layer to determine the language of the audio, meaning the output layer consists of as many output neurons as available languages.
...
\begin{axis}[
	...
	ymajorgrids=true,
	grid style=dashed,
	legend entries = {Smoothed, Original}
]
\addplot[
	color=blue,
	mark=square,
