...
This setup improved error rates from 0.256 down to 0.245, an improvement of 1.2 \%.
...
As a further experiment, we used the pre-trained nets of the stacked hidden layers of the euronews net and finetuned them with the cross-set of Lecture-Data respectively. This was done by loading net-parameters from pre-training and training the final output layer with a low learning rate of 0.1 (1/10th of the learning rate we used in the normal setup to make meaningful learning possible while fine-tuning) using the cross-set. It had the expected result of improving cross-set results for the net, which can be seen in Table~\ref{tab:LIDFine} for the fine-tuning attempts. Since the Lecture Data corpus only has 3 languages, we couldn't evaluate this set using all euronews languages but still gave a figure for the net results with the euronews corpus for English, German and French.

For this experiment we used the tree-net 6-layer structure of the euronews nets, just omitting the output layer, and then used the lecture data corpus to train the output layer with only 3 output neurons. This yielded an improvement of 7.4\% for the error of the tree-net euronews net recognizing the lecture data development set.
%TODO
A second experiment was conducted, using the same base euronews net, but adding 2 layers on top for the finetuning with 200 and 100 neurons respectively. Which further improved the results, by another . See~\ref{tab:LIDFIne} for the results.
...
\begin{tabular}{| l | c | r | }
	\hline
	\textbf{Net Structure} & \textbf{Euronews Error} & \textbf{Cross-Set-Error}  \\
	\hline
	Tree-net Euronews net w/o finetuning & 0.242 &  0.130 \\
	Tree-net Euronews net with finetuning & - & 0.105 \\
	Tree-net Euronews net with finetuning 2 layers & & 0. \\
	\hline
	\textbf{Change} & \textcolor{red}{} & \textcolor{green}{0.074} \\
	\hline
\end{tabular}
\caption{Results of the different net structures on the Euronews corpus. Frame-based errors on train/validation set.}
\end{table}
...
\begin{tabular}{| l | r |}
	...
	\hline
\end{tabular}
\caption{Results of the best net structure (tree-structure 6-layer) evaluated with all the tried post-filtering approaches.}
\end{table}
